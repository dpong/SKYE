...
class Build_model():
    ...
    def build_model(self, state_size, self_feat_shape, neurons, action_size, atoms):
        ...
        con1 = Conv1D(state_size[1], 5, padding='causal', activation='relu')(norm)
        con2 = Conv1D(state_size[1], 5, padding='causal', activation='relu')(con1)
        #pool_max = MaxPooling1D(pool_size=5, strides=1, padding='same')(con2)
        flat = Flatten()(con2)
        # 外插 self_state_input
        connect = concatenate([flat, flat_s1])
        # 連結層
        ...
        for i in range(action_size):
            duel_distribution_list_a.append(
                NoisyDense(atoms, activation='relu', name='a_{}'.format(i))(n1)
                )
        # deuling 計算 value     
        #value_in = tf.concat([t for t in norm_list_v], 1)
        value = NoisyDense(atoms, activation='relu', name='value')(n1)
